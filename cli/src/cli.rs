...
pub enum CliError {
    ...
    #[error("Account {1} has insufficient funds for fee ({0} KUNCI)")]
    InsufficientFundsForFee(f64, Pubkey),
    #[error("Account {1} has insufficient funds for spend ({0} KUNCI)")]
    InsufficientFundsForSpend(f64, Pubkey),
    #[error("Account {2} has insufficient funds for spend ({0} KUNCI) + fee ({1} KUNCI)")]
...
mod tests {
    ...
    fn test_cli_process_command() {
        ...
        assert_eq!(process_command(&config).unwrap(), "0.00000005 KUNCI");
    ...
    fn test_parse_transfer_subcommand() {
        ...
        //Test Transfer Subcommand, KUNCI
